...
int main( int argc, char** argv )
{
    ...
    char c[] = "/home/kevin/下载/remode_test_data/test_data";

    // 从数据集读取数据
    vector<string> color_image_files;
    vector<SE3> poses_TWC;
    bool ret = readDatasetFiles( c, color_image_files, poses_TWC );
    if ( ret==false )
    {
    ...
    for ( int index=1; index<color_image_files.size(); index++ )
    {
        ...
        SE3 test_ref = SE3( Quaterniond(0.174771, 0.0174794, -0.00189307, 0.984452),
                            Vector3d(-0.159017, 0.058306, 0.346453));
        SE3 test_curr = SE3( Quaterniond(0.131672, 0.015136, -0.00080867, 0.991178),
                             Vector3d(-0.16359, 0.0442509, 0.34563));
        SE3 test_pose_T_C_R = test_curr.inverse() * test_ref;

        update( ref, curr, test_pose_T_C_R, depth, depth_cov );
        plotDepth( depth );
        imshow("image", curr);
        waitKey(1);
    }
...
)
{
    ...
    Vector2d pt_test1(116.453, 507.617);
    Vector2d pt_test2(206.531, 495.176);
    Vector3d f_ref = px2cam( pt_test1 );
    f_ref.normalize();
    Vector3d f_curr = px2cam( pt_test2 );
    f_curr.normalize();
